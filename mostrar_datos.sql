...
FROM marca m 
JOIN modelo m2 ON m.idmodelo = m2.id 
join grupo_empresarial ge on ge.idmarca = m.id 
join coche c on m.id = c.idmarca
join color c2 on c.idcolor = c2.id 
join aseguradora a on c.idaseguradora = a.id 